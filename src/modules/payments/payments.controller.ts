...
export async function initializePaymentController(
  ...
  try {
    ...
    const paystackTransaction: PaystackTransaction =
...
export async function getPaymentController(
  ...
  try {
    ...
    const payment =
      await findPaymentByReferenceForUser(
        reference,
        req.user.userId
      );
