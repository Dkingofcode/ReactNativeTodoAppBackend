...
export async function createTodoController(
  ...
  try {
    ...
    // 4. Verify payment
    const payment = await findSuccessfulPaymentForUser(
      paymentId,
      userId
    );
    ...
    if (!payment) {
      return res.status(403).json({
        message:
          "A successful payment belonging to this user is required to create a Todo.",
      });
    }
...
export async function getTodoByIdController(
  ...
  try {
    ...
    return res.status(500).json({
      message:
        "Something went wrong while fetching the Todo.",
    });
  }
}
  ...
  try {
    ...
    if (title !== undefined) {
      ...
      if (cleanTitle.length > 255) {
        return res.status(400).json({
          message:
            "Todo title cannot exceed 255 characters.",
        });
      }
    }
